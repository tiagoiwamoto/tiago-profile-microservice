...
@Slf4j
@AllArgsConstructor
public class MeCertificateDeleteUseCase {

    private final MeCreateUpdateUseCase meCreateUpdateUseCase;
    private final MeRecoveryUseCase meRecoveryUseCase;

    public ResponseDto prepareToRemoveMeCertificate(String uuid, String certificateId){
        Me me = this.meRecoveryUseCase.validIfRecordExists(uuid);
        Optional<Certificate> optionalResume =
                me.getCertificates().stream().filter(line -> line.getId().equalsIgnoreCase(certificateId)).findFirst();
        if(optionalResume.isPresent()){
            Certificate certificate = optionalResume.get();
            me.getCertificates().remove(certificate);
